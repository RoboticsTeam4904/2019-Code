...
public class DefaultOperator extends Operator {
	...
	public void bindCommands() {
		RobotMap.HumanInput.Operator.joystick.button1
			.onlyWhileHeld(new ElevatorSetSpeed(RobotMap.HumanInput.Operator.joystick));
		...
		RobotMap.Input.elevatorSwitchBottom.whenPressed(new SingleOp(() -> {
			RobotMap.Component.elevCANEncoder.resetViaOffset();
		}));
		RobotMap.Input.elevatorSwitchTop.whenPressed(new SingleOp(() -> {
			RobotMap.Component.elevCANEncoder.resetViaOffset(RobotMap.Metrics.ELEVATOR_INNER_HEIGHT);
		}));
	}
}